...
access_ensure_project_level( plugin_config_get( 'kanban_view_threshold' ) );
 ...
 * example:
 * $columns = array(
 *	plugin_lang_get('header_column_1') => array('status' => array(10), 'wip_limit' => 0),
...
$columns = array(
	plugin_lang_get('header_column_1') => array('status' => array(10), 'wip_limit' => 0),
	plugin_lang_get('header_column_2') => array('status' => array(30), 'wip_limit' => 0),
	plugin_lang_get('header_column_3') => array('status' => array(40), 'wip_limit' => 8),
	plugin_lang_get('header_column_4') => array('status' => 20, 'wip_limit' => 0),
	plugin_lang_get('header_column_5') => array('status' => 50, 'wip_limit' => 8),
	//plugin_lang_get('header_column_6') => array('status' => array(60,80,90), 'wip_limit' => 0),
);
...
<div class="col-md-12 col-xs-12">
    ...
    <div class="widget-box widget-color-blue2">
        ...
        <h4 class="widget-title lighter">
            ...
            <?php echo plugin_lang_get( 'kanban_link' ) . ' - ' . project_get_name($t_project_id) ?>
        </h4>
    </div>
    ...
    <div class="widget-body">
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-condensed kanbanTable" cellspacing="0" cellpadding="0">
                <tr>
                    <td colspan="<?php echo count($columns)-2;?>">
                        <?php echo plugin_lang_get( 'sort' ); ?> 
                        <a href="plugin.php?page=MantisKanban/kanban_page&sort=last_updated" <?php if($f_sort_by == 'last_updated') {?> class="bold"<?php }?>><?php echo plugin_lang_get( 'sort_date_modified' );?></a>
                         |
                        <a href="plugin.php?page=MantisKanban/kanban_page&sort=priority" <?php if($f_sort_by == 'priority') {?> class="bold"<?php }?>><?php echo plugin_lang_get( 'sort_priority_link' );?></a>
                    </td>
                    <td colspan="2">
                        <?php if( helper_get_current_project() == 0 ) { ?>
                            <?php echo plugin_lang_get( 'projectdisplay' ); ?>
                            <a href="plugin.php?page=MantisKanban/kanban_page&pdisplay=combined"><?php echo plugin_lang_get( 'project_nogroups' );?></a> |
                            <a href="plugin.php?page=MantisKanban/kanban_page&pdisplay=splitted"><?php echo plugin_lang_get( 'project_groups' );?></a>
                        <?php } ?>
                    </td>
                </tr>
                <?php
                    ...
                    foreach($all_project_ids as $curr_project_id) {
                        if( sizeof($all_project_ids) > 0 && $curr_project_id == 0 ) {
                ?>
                <tr>
                ...
                <tr>
                    ...
                    <td valign="top"
                        ...
                        <h2 <?php if( isset( $column['color'] ) ) { ?>style="background-color:<?php echo $column['color']; ?>"<?php } ?>>
                            <?php
                            echo $title . ' ( <span '; 
                            if($column['wip_limit'] > 0 && $rowcounts[$title] > $column['wip_limit']){ 
                                echo 'class="wip_exceeded"'; 
                            } 
                            else { 
                                echo 'class="wip_ok"'; 
                            } 
                            echo ' >' . $t_bug_count . '</span>';
                            if($column['wip_limit'] > 0) {
                                echo " / " . $column['wip_limit'];
                            } 
                            echo ' )';
                            ?>
                        </h2>
